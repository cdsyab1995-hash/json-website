...
'''
    ...
    faq_patterns = [
        r'(<div class="section[^"]*faq-section[^"]*">)',
        r'(<div class="faq-container">)',
        r'(<section[^>]*id="faq"[^>]*>)',
        r'(</details>\s*</div>\s*</section>)',  # escape/extract 的 FAQ 结构
    ]
    ...
    # 如果找不到 faq，在 article-content 后插入
    if 'article-content' in content:
        content = content.replace('</div>\n</div>\n</article>', usecases_html + '</div>\n</div>\n</article>')
        return content, True
    
    # 最后尝试：找 </section> 后面跟 main 的位置
    match = re.search(r'(</section>\s*</main>)', content)
    if match:
        content = content.replace(match.group(1), usecases_html + match.group(1), 1)
        return content, True
...
def process_page(page):
    ...
    if content != original:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"  [OK] {page} (UseCases={has_usecases})")
        return True
    else:
...
def main():
    print("=== Adding Use Cases Section ===\n")
    updated = 0
    for page in PAGES:
