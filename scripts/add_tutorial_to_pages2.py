...
for fname in ['blog.html', 'hash-generator.html', 'uuid-generator.html']:
    ...
    # Check if already has Tutorial
    if '>Tutorial</a>' in content or '>Tutorial<' in content:
        print(f'{fname}: already has Tutorial')
        continue
    
    # Pattern: after nav-dropdown closes, before <!-- Right-side links -->
    old_pattern = '''                </div>
                </div>
            </div>
            <!-- Right-side links -->
