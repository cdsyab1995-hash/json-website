...
# Blog navbar (no Tutorial)
BLOG_NAVBAR = STANDARD_NAVBAR.replace('''
            <a href="https://www.aijsons.com/tutorial" class="nav-link">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path>
                    <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path>
...
def replace_navbar(filepath, new_navbar):
    ...
    # Try different patterns to find navbar
    patterns = [
        (r'<nav class="navbar">.*?</nav>', 'nav'),
        (r'<div class="navbar-links">.*?</div>\n    </nav>', 'navbar-links'),
        (r'<header class="navbar">.*?</header>', 'header'),
    ]
    ...
    for pattern, name in patterns:
        ...
        if m:
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f'  Fixed: {os.path.basename(filepath)} ({name} pattern)')
            return True
    
    print(f'  WARNING: Could not find navbar in {os.path.basename(filepath)}')
    return False

# Fix css-minifier.html
print('=== css-minifier.html ===')
replace_navbar(os.path.join(pages_dir, 'css-minifier.html'), STANDARD_NAVBAR)

# Fix html-encoder.html
print('\n=== html-encoder.html ===')
replace_navbar(os.path.join(pages_dir, 'html-encoder.html'), STANDARD_NAVBAR)

# Fix jwt-decoder.html
print('\n=== jwt-decoder.html ===')
replace_navbar(os.path.join(pages_dir, 'jwt-decoder.html'), STANDARD_NAVBAR)

# Fix blog.html (no Tutorial)
print('\n=== blog.html ===')
replace_navbar(os.path.join(pages_dir, 'blog.html'), BLOG_NAVBAR)
